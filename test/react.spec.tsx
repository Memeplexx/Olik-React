...
import { createStore, derive, importOlikAsyncModule } from 'olik';
import { expect, beforeAll, it, afterEach } from 'vitest';
...
it('should create and update a store', () => {
  const select = createStore({ state: initialState });
...
it('should useDerivation with deps', async () => {
  const get = createStore({ state: initialState });
  let calcCount = 0;
  const App = () => {
  ...
  await render(<App />);
  await expect(screen.getByTestId('result').textContent).toEqual(initialState.string + initialState.object.property + 0);
  await expect(calcCount).toEqual(1);
  await (screen.getByTestId('btn-1') as HTMLButtonElement).click();
  await expect(calcCount).toEqual(1);
  await (screen.getByTestId('btn-2') as HTMLButtonElement).click();
  await expect(calcCount).toEqual(2);
});
...
it('should respond to async queries', async () => {
  ...
  const App = () => {
    const future = select.object.property.$set(fetchString).$useFuture();
    return (
      <>
